    ...
    public class SheetComparer {
        ...
        public IEnumerable<IChange> Compare() {
            ...
            if (prevIsMulti == upIsMulti) {
                ...
                Console.Error.WriteLine("Type of sheet {0} has changed, unable to detect changes.",
        ...
        private static IEnumerable<IChange> CompareVariant2(ISheet previousSheet,
                                            ...
                                            Language language,
                                            ColumnMap[] columns) {
            var prevRows = previousSheet.Cast<RelationalDataRow>().SelectMany(r => r.SubRows).ToArray();
            var updatedRows = updatedSheet.Cast<RelationalDataRow>().SelectMany(r => r.SubRows).ToArray();
